...
> You are reading the `kirkstone` branch. It targets the OE-Core **4.x** release series: kirkstone (LTS) plus langdale, mickledore, and nanbield. For other series, switch to the matching release branch (`scarthgap` for 5.x, `dunfell` for 3.x); for 6.x use `main`.
